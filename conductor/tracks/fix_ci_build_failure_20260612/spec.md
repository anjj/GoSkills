...
- Inject `VITE_MICROSOFT_TENANT_ID` into the Docker build process and the CI/CD workflow.
...
6. `Dockerfile` and `.github/workflows/ci.yml` accept and pass `VITE_MICROSOFT_TENANT_ID` at build time.
